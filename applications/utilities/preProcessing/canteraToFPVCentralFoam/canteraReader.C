...
/*void Foam::canteraReader::calculatedQ()
{
   Info << "calculate Heat Release Rate terms" << endl;
   ...
   for (int i=0;i<singleData_[labelZ].size();i++)
   {
      for (int j=0; j<speciesName_.size();j++)
      {
         word speNPortion = "dQ"+speciesName_[j];
   ...
   singleData_.append(dQ);
}*/

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::canteraReader::canteraReader(const IOdictionary& canteraDict, const IOdictionary& sPortionDict, psiReactionThermo& thermo, basicSpecieMixture& composition) //amended
...
void    Foam::canteraReader::writePv(Foam::IOdictionary& dictionaryChi, Foam::OFstream& output)
{
